...
1. **글 저장** -- `python3 main.py --save <URL>` 한 줄로 자동 추출 + 분류 + vault 저장. AI 없이도 동작.
...
1. **글 저장** — `python3 main.py --save <URL>` (또는 아무것도 안 해도 트렌드 상위 3개가 자동 저장)
2. **텔레그램 확인** — 브리핑이 자동 도착

끝. 직접 저장 안 해도 매일 트렌드 상위 3개가 vault에 자동 축적됩니다.
