...
주간 지식 컴파운딩 리포트가 루만이 수작업으로 했던 일을 합니다 -- 당신이 미처 보지 못한 노트 사이의 연결을 발견합니다. "이번 주 저장한 3개 글이 모두 'X'라는 공통 주제를 다루고 있습니다." 월간 메타 리뷰는 지식 수집의 사각지대를 추적합니다.

#### 3. 복리 학습 (Farnam Street -- Shane Parrish)

"지식은 복리처럼 쌓인다. 단, 능동적으로 리뷰하고 연결해야만."

대부분의 사람은 글을 저장하고 다시 보지 않습니다. 주간 리포트는 저장한 것을 나열하는 게 아니라, **패턴을 발견하고, 프로젝트에 적용할 방법을 제안하고, 다음 주에 탐색할 키워드를 추천**합니다. 매주가 이전 주 위에 쌓입니다.
...
메타 리뷰가 핵심 혁신입니다: **시스템이 스스로를 개선합니다.** 어떤 카테고리를 소홀히 하고 있는지, 어떤 출처가 노이즈인지, AI가 제안한 아이디어가 실제 코드 커밋으로 이어졌는지를 추적합니다.
